...
int platform_thread_create(platform_thread_t *thread, void *(*func)(void *), void *arg, const char *name){
    ...
    ctx->func = func;
    ctx->arg  = arg;
    BaseType_t rc = xTaskCreate(_platform_task_wrapper, name ? name : "task", 4096, ctx, 5, thread);
    if (rc != pdPASS) { 
        free(ctx); 
        return -1; 
    }
...
#else
    // Fallback for standard PC environments / alternative hardware
...
{
    time_t now;
    time(&now); // returns the current epoch time from internal RTC
    return (unsigned long)now;
}