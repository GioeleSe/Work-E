...
typedef enum {
    TRUNK_IDLE,
    TRUNK_CLOSING, // trackin only closing movement as it's the only one with a limit switch
    TRUNK_READY,
    TRUNK_ERROR
} TrunkState_t;

#define PIN_SERVO_CONTROL 13
#define PIN_RADAR_SDA 27
#define PIN_RADAR_SCL 26
#define SERVO_STOP_US           1500
#define SERVO_CW_US             1300
#define SERVO_CCW_US            1700
#define SERVO_DEG_PER_MS_CW     0.25f
#define SERVO_DEG_PER_MS_CCW    0.22f  
#define RADAR_CONE_DEG 30
#define RADAR_STEP_DEG 7
...
typedef struct Beta_t {
    ...
    int object_loader = 0;
    int object_unloader = 1; // trunk function
